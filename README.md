...
Wheel deltas are normalised across the pixel, line and page units browsers
report, and a trackpad pinch -- which arrives as a wheel event carrying
`ctrlKey`, not as a touch gesture -- gets its own, shorter step, because a
whole pinch accumulates only a couple of hundred pixels where a mouse notch is
a hundred on its own.
